...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!botToken) {
      return new Response(
        JSON.stringify({ ok: false, error: "Bot token is required" }),
        {
          status: 400,
          headers: {
            ...corsHeaders,
    ...
    if (!method) {
      return new Response(
        JSON.stringify({ ok: false, error: "Method is required" }),
        {
          status: 400,
          headers: {
            ...corsHeaders,
    ...
    console.log(`Proxying request to Telegram API: ${method}`);
